...
# gstreamer-plugins-base 0.10.36 for el8/el9: the base plug-in set that goes
# with the gstreamer 0.10.36 package in this directory.
#
# Provenance: the RHEL7/CentOS 7 packaging (gstreamer-plugins-base-0.10.36-10.el7
# src.rpm, /usr/share/doc and the spec are Fedora's) plus the upstream tarball
# https://gstreamer.freedesktop.org/src/gst-plugins-base/gst-plugins-base-0.10.36.tar.xz
# whose sha256 (1fe45c3894903001d4d008b0713dab089f53726dcb5842d5b40c2595a984e64a)
# is byte-identical to the tarball inside that SRPM.
#
# Naming follows Red Hat's own scheme for the two GStreamer generations
# side by side (verified against the CentOS 7 vault: gstreamer 0.10 is the
# plain name, 1.x is gstreamer1, and the plug-in sets mirror that), so this
# package is gstreamer-plugins-base while el8/el9's own is
# gstreamer1-plugins-base.  compat-gstreamer0-plugins-base is offered as an
# alias only because this repository keeps the 0.10 stack in the
# compat-gstreamer0/ directory; a plain gstreamer-plugins-base install is what
# software linked against the 0.10 ABI needs.
#
# Changes vs the el7 spec: https source URL (COPR fetches sources with
# --proto -all,+https,+ftps), no documentation build (gtk-doc), liboil dropped
# (gone from el8/el9; orc is used instead), libgudev1-devel renamed to
# pkgconfig(gudev-1.0), the el6/el7-era Obsoletes for the 0.8 packages dropped,
# %%ldconfig_scriptlets instead of %%post -p /sbin/ldconfig, and the plug-in
# list kept as-is until the build says otherwise.
...
# search alias for this repository's directory name, see the header comment
Provides:       compat-gstreamer0-plugins-base = %{version}-%{release}
Requires:       iso-codes
...
Patch0: gstreamer-plugins-base-0.10.36-mpegaudioversion.patch
# glib-mkenums writes the enum include list as "\#include" -- the backslash is
# the make-level escape used in the enum_headers variable.  With make 4.3 and
# glib 2.68 (el9) it survives into the generated *-enumtypes.c, where it is no
# longer a preprocessor directive, and the build dies with
#   interfaces-enumtypes.c:228:9: error: 'GST_TUNER_CHANNEL_AUDIO' undeclared
# el8 (make 4.2, glib 2.56) strips it and builds fine.  Filter the generated
# output so both do.
Patch1: gstreamer-plugins-base-0.10.36-enumtypes-include.patch
...
%build
%configure \
  --with-package-name='GStreamer plugins-base 0.10 compat package' \
  --with-package-origin='https://copr.fedorainfracloud.org/coprs/vowstar/eda/' \
  --enable-experimental \
  --disable-gtk-doc \
  --enable-orc \
  --disable-gnome_vfs \
